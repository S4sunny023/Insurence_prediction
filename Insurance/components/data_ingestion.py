...
class DataIngestion: #data divided into train, test & validation
    ...
    def initiate_data_ingestion(self)->artifact_entity.DataIngestionArtifact:
        try:
            ...
            logging.info("savinge data for feature store folder if not available")
            feature_store_dir = os.path.dirname(self.data_ingestion_config.feature_store_file_path)
            os.makedirs(feature_store_dir,exist_ok=True)
            logging.info('saving data to feature store folder')
            df.to_csv(path_or_buf=self.data_ingestion_config.feature_store_file_path,index=False,header=True)

            logging.info('splitting  data into train and split')
            ...
            logging.info("save dataset to feature folder")
